...
struct xLine {
    ...
    bool f;    //f = true表示这条边是这个四边型的左边那条边（入边），f = false表示出边
...
void Up_sum(int t) {    //更新区间内被覆盖的长度
    if(node[t].cover > 0)   node[t].sum = node[t].len;
...
void Up_lsum(int t) {    //更新区间被没覆盖的线段长度    
    if(node[t].cover > 0) {
        ...
        node[t].lsum = node[L(t)].lsum + node[R(t)].lsum - node[L(t)].rcover*node[R(t)].lcover;
    } else {
        node[t].lcover = node[t].rcover = false;
        node[t].lsum = 0;
    }
...
void updata(int t, int l, int r, bool c) {    //更新线段树，c = 1表示插入线段，c = 0表示删除线段
    ...
    else if(l >= mid)    updata(R(t), l, r, c);
    else {
        updata(L(t), l, mid, c);
        updata(R(t), mid, r, c);
    }
...
int find(int x) {    //给y轴上的点重新编号，传说中的离散化
    ...
    while(l <= r) {
        mid = MID(l, r);
        if(ind[mid] == x)   return mid;
        else if(ind[mid] > x)   r = mid - 1;
        else  l = mid + 1;
    }
    return 0;
}
...
int main() {
    ...
    while(~scanf("%d", &n)) {
        ...
        REP(i, n) {
            ...
            Line[i*2].x = x1; Line[i*2].y1 = y1; Line[i*2].y2 = y2; Line[i*2].f = 1;    //入边
            Line[i*2+1].x = x2; Line[i*2+1].y1 = y1; Line[i*2+1].y2 = y2; Line[i*2+1].f = 0;    //出边
            ind[i*2] = y1;    
        ...
        for(i = 0; i < n*2 - 1; ++i) {    //删掉多余的点
            if(ind[i] != ind[i+1])  ind[m++] = ind[i+1];
        }
        /*for(i = 0; i < m; ++i)
            printf("%d ", ind[i]);
        cout << endl;*/
        ...
        int presum = 0;
        for(i = 0; i < 2*n - 1; ++i) {    
            if(Line[i].f)   updata(1, find(Line[i].y1), find(Line[i].y2), 1);    //入边则insert
            else    updata(1, find(Line[i].y1), find(Line[i].y2), 0);    //出边则delete

            ans += node[1].lsum*(Line[i+1].x - Line[i].x)*2;    //这个累加平行与x轴的边
            ans += iabs(node[1].sum - presum);    //累加平行与y轴的边，画画图就可以看清楚了
            presum = node[1].sum;
        }
        updata(1, find(Line[2*n-1].y1), find(Line[2*n-1].y2), 0);    //别忘了最后一个出边要删除
